...
export default function PhotoAnalysisStudio() {
  ...
  const [initialResult, setInitialResult] = useState<PhotoAnalysisResponse | null>(null)
  ...
  return (
    <div className="photo-analysis-page">
      ...
      {initialResult && (
        <ResultSummary title="초기 감지 결과" result={initialResult} />
      )}

      {initialResult && (
        <section className="photo-action-panel">
          <div>
            <h3>2. 랜드마크 수정 후 최종 분석</h3>
            <p>포인트를 원하는 위치로 조정한 다음 최종 분석을 실행하면 저장 가능한 결과를 다시 받습니다.</p>
          </div>
          <button
            className="photo-primary-button"
          ...
          >
            {finalAnalyzeMutation.isPending ? '최종 분석 중...' : '2. 최종 분석 실행'}
          </button>
        </section>
      )}
